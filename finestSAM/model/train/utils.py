...
def plot_image_with_mask_and_points(
    image: np.ndarray,
    ...
    """
    Plot the image with the mask and points overlaid.
    """
    ...
    if points is not None:
        ...
        if labels is not None:
            ...
            plt.scatter(pos_points[:, 0], pos_points[:, 1], color='green', marker='*', s=100, edgecolor='white', linewidth=1.25)
            plt.scatter(neg_points[:, 0], neg_points[:, 1], color='red', marker='*', s=100, edgecolor='white', linewidth=1.25)
        else:
            plt.scatter(points[:, 0], points[:, 1], color='green', marker='*', s=100, edgecolor='white', linewidth=1.25)
